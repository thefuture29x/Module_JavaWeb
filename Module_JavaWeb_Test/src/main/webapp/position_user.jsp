...
<html>
<head>
    <title>Find User with Position</title>
...
<div class="container">
    ...
    <br/>
    <form action="exercises-e" method="post">
        <c:forEach items="${listPosition}" var="position">
            <button type="submit" class="btn btn-primary" value="${position.id}" name="id_postition" >${position.position}</button>
        </c:forEach>
    </form>
    <c:if test="${not empty messageResponse}">
        <div class="alert alert-${alert}" role="alert" align="center">
